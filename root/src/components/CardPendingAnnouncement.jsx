...
import {updatePendingAnnouncementFunc} from '../states/penRejState'
        ...
        <div>
            <div className='p-3 rounded-5 bg-light border w-100 mx-4 my-2'>
                ...
                <div className='mb-5 mt-4 d-flex justify-content-center gap-5'>
                    {!singleData[0] || isLoading ? <Placeholder.Button xs={4} aria-hidden="true" /> : <Button variant="danger">Reject</Button>}
                    {!singleData[0] || isLoading ? <Placeholder.Button xs={4} aria-hidden="true" /> : <Button variant="success" onClick={() => { window.alert(); dispatch(updatePendingAnnouncementFunc({ ...singleData[0], status: 1 })) }}>Approve</Button>}
                </div>
            </div>
