                ...
                isMyAnnouncement && typeof (room) !== "object" ?
                    <div className='position-relative d-flex justify-content-center pt-1'>
                        <div className='pb-3 d-flex align-items-center flex-column position-absolute'>
                            {
                                usersById && usersById.map((el, index) => {
                                    if (el.id == room.split("$")[2])
                                        return (
                                            <div key={`chatannouncement1-${index}`} className='d-flex gap-2 align-items-center myCursor' onClick={() => setRoom(initialRoomSet)}>
                                                <i className="bi bi-chevron-left display-6"></i>
                                                <div className='pe-3' onClick={() => setRoom(`${singleData.idOwner}$${singleData.id}$${el.id}`)}>
                                                    <CardChatUser idAnn={singleData.id} user={el} />
                                                </div>
                                            </div>
                                        )
                                })
            ...
            <div className={`w-100 myBgWhite p-3 myOverflowY myVh70`}>
                {
                    typeof (room) === "object" ?
                        usersById && usersById.map((el, index) => {
                            return (
                                <div key={`chatannouncement2-${index}`} className='border bg-light mt-1 myCursor' onClick={() => setRoom(`${singleData.idOwner}$${singleData.id}$${el.id}`)}>
                                    <CardChatUser idAnn={singleData.id} user={el} />
                                </div>
                            )
                        })
