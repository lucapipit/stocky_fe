...
function _FormAnnouncement() {
  ...
  const formCheck = async () => {
    ...
    const formFilled = brandName && manufacturerName && modelName && quantity && file && description && categoriesProduct;
    ...
    <div className='d-flex justify-content-center'>
      <div className='myMaxW1000 w-100 mt-5'>
        <form encType='multipart/form-data'>
            ...
            <DropdownButton title={categoriesProduct.length ? categoriesProduct.length === 1 ? "1 item" : `${categoriesProduct.length} items` : "select one or more categories"}>
              ...
              <hr className='w-100 px-5 mt-1' />
              {
                productCategories && productCategories.map((el) => {
                  if (el.area === "dental") { return <span className="text-light p-1 px-3 m-1 rounded-5 myBgAcqua myCursor myBorderAcqua" onClick={(e) => { manageCategory({ el, e }) }}>{el.eng}</span> }
                })
              }
              ...
              {
                productCategories && productCategories.map((el) => {
                  if (el.area === "medical") { return <span className=" text-light p-1 px-3 m-1 rounded-5 myBgRed myCursor myBorderRed" onClick={(e) => { manageCategory({ el, e }) }}>{el.eng}</span> }
                })
              }
            </DropdownButton>
          ...
          <div className='d-flex justify-content-center'>
            <Button className='mb-5' onClick={formCheck}>{totalPrice !== "0" ? "Checkout" : "Publish"}</Button>
          </div>
          {
